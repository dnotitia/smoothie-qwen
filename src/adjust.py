...
class WeightAdjuster:
    ...
    def compute_scale_factor(self, 
                            weighted_prob: float, 
                            log_base: float, 
                            minimum_scale_factor: float) -> float:
        """
        ...
        if weighted_prob <= 0:
            ...
            return 1.0 - (1.0 - minimum_scale_factor) * (np.log(1 + (log_base - 1) * weighted_prob) / np.log(log_base))
    
    def modify_weights(self, 
                      ngram_weights: List[float] = [0.7, 0.2, 0.1], 
                      target_scale_factor: float = 0.01,
                      log_base: float = 100, 
                      minimum_scale_factor: float = 0.01) -> int:
        """
        ...
        self.logger.info(f"Settings: n-gram weights={ngram_weights}, log_base={log_base}, min_scale={minimum_scale_factor}")
        ...
        self.logger.info(f"Normalized n-gram weights: 2-gram={normalized_weights[0]:.2f}, "
        ...
        with torch.no_grad():
            ...
            self.logger.info(f"Reducing weights of {len(self.target_tokens):,} target tokens by factor {target_scale_factor:.4f}")
            ...
            self.logger.info(f"Processing {len(self.token_analysis):,} analyzed tokens...")
            
            for token_id, probs in tqdm(self.token_analysis.items(), desc="Processing analyzed tokens"):
        ...
        self.logger.info(f"Weight modification completed: {modified_count:,} tokens modified")
